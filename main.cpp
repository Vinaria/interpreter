...
enum type_of_lex {
    LEX_NULL,                                                                                   /* 0*/
    LEX_AND, LEX_BOOLEAN, LEX_STRING, LEX_DO, LEX_IF, LEX_FALSE, LEX_INT,                       /* 7*/
    LEX_NOT, LEX_OR, LEX_PROGRAM, LEX_READ, LEX_TRUE, LEX_CONT, LEX_WRITE,                      /*14*/
    LEX_WHILE,
    LEX_FIN,                                                                                    /*16*/
...
Lex Scanner::get_lex () {
  ...
  do {
    ...
    switch ( CS ) {
      case H:
        if ( c==' ' || c == '\n' || c== '\r' || c == '\t' );
        else if ( isalpha (c) ) {                            // неотр значение если символ - буква
          buf.push_back (c);
          CS  = IDENT;
        }
        else if ( isdigit (c) ) {
...
class Parser {
    ...
    void  dec ( type_of_lex type);      // context analysis
    void  check_id ();
    void  check_op ();
    void  check_not ();
    void  eq_type ();
...
void Parser::check_op () {
  ...
  //cout << "Finished checking operators\n";
}
    ...
    switch ( pc_el.get_type () ) {
      ...
      case LEX_READ:
        ...
        if ( TID[val].get_type () == LEX_NUM ) {
          int k;
          //cout << "Input int value for " << TID[val].get_name () << endl;
          cin >> k;
          TID[val].put_value (k);
        }
          ...
          int k;
          string s;
          //cout << "Input boolean value (true or false) for " << TID[val].get_name() << endl;
          cin >> s;
          if ( s != "true" && s != "false" )
          ...
          string s;
          //cout << "Input string value for " << TID[val].get_name() << endl;
          cin >> s;
